...
select * from product where product_name ='진득한 포도137';
select * from product_review where product_no =77;
...
insert into product_review values(review_seq.nextval,4,1,'진득하니 최고',2.4,sysdate);
commit;


SUBSTR(TO_CHAR((FLOOR(2 * r2.star_avg2) / 2) , '9.9'), 1, 2) || '-' ||SUBSTR(TO_CHAR((FLOOR(2 * r2.star_avg2) / 2) , '9.9'), 4, 4) as starClass,
TO_CHAR(r2.star_avg2 , '9.9') as starAvg2,
...
TO_CHAR(r2.order_price , '999,999,999,999,999') as orderPrice from
;
...
where product_no in (21,1,2);

;
select * from homecook_deal;

update homecook_deal set discountend_date = sysdate where deal_no = 1;
