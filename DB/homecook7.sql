...
commit;
select * from product_review where product_no = 1 order by review_no desc;
update product_review set star=1 where review_no = 66;

select * from qna ;
select * from product_review;
update homecook_deal set discountend_date = sysdate where product_no = 1;
...
s
insert into product_like values(product_like_seq.nextval,1,1);



commit;

select * from product_img where main_img = 'N';


select * from qna where product_ref_no = 1 and member_no =4;
...
update product set stock = 30 where product_no = 1;
commit;
select * from product;